...
if __name__ == "__main__":
    ...
    database_root = './baidu_hongkong_cycle200.db'                               # 设置数据库的路径
    ...
    count = 0
    while get_db_count(database_root)[1] > 0:                     # 循环, temp有数据就一直采集
        svid = fetch_and_delete_svid_from_temp(database_root)     # 从temp表中获取一个svid并删除它
        svid_2_collect(svid)                                      # 根据svid采集相邻街景并保存
        time.sleep(random.randint(0, 8))                          # 设置每次采集的休眠时间

        print(f"[-] 共采集 {get_db_count(database_root)[0]} 个街景点坐标, {time.strftime('%Y-%m-%d %H:%M:%S')}")
