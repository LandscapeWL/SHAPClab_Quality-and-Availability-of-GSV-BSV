...
if __name__ == "__main__":
    ...
    database_root = './google_hongkong_cycle100.db'                               # 设置数据库的路径
    ...
    while get_db_count(database_root)[1] > 0:                     # 循环, temp有数据就一直采集
        panoid = fetch_and_delete_panoid_from_temp(database_root) # 从temp表中获取一个panoid并删除它
        panoid_2_collect(panoid)                                  # 根据panoid采集相邻街景并保存
        time.sleep(10)                         # 设置每次采集的休眠时间
